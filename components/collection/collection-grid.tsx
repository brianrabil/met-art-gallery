...
import { useFilteredCollection, usePrefetchObject } from "@/lib/queries"
import type { CollectionItem } from "@/lib/types"
import { useQueryClient } from "@tanstack/react-query";
import { queryKeys } from "@/lib/constants";
import { getFilteredCollection } from "@/lib/actions";
...
export default function CollectionGrid({ viewType }: CollectionGridProps) {
    ...
    <div className="space-y-8">
      {viewType === "grid" ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {data.items.map((item) => (
            <CollectionItemCard key={item.id} item={item} onQuickView={() => setSelectedItem(item)} />
          ))}
        </div>
      ) : (
        <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-6 space-y-6">
          {data.items.map((item) => (
            <div key={item.id} className="break-inside-avoid mb-6">
              <CollectionItemCard item={item} onQuickView={() => setSelectedItem(item)} />
            </div>
          ))}
        </div>
      )}
...
function CollectionItemCard({ item, onQuickView }: { item: CollectionItem; onQuickView: () => void }) {
...
// Helper function for prefetching collection data
function usePrefetchCollection(options: CollectionFilterOptions) {
